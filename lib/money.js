...
(function () {
  ...
  function Currency(name, fractionDigits) {
    if (fractionDigits < 0) {
      throw new RangeError('Currency: fractionDigits can\'t be lesser than 0');
    }
  ...
  function _moneyFactory(amount, currency) {
  ...
  Money.prototype.add = function (money) {
    ...
    return _moneyFactory(this.amount + money.amount, this.currency);
  };
  ...
  Money.prototype.mult = function (factor) {
    ...
    return _moneyFactory(Math.round(this.amount * factor), this.currency);
  };
  ...
  Money.prototype.allocate = function (weights) {
    ...
    for (i = 0; i < weights.length; i += 1) {
      allocation.push(_moneyFactory(Math.floor(this.amount * weights[i] / total), this.currency));
